...
- `PRD.md` — 전체 제품 명세(FR-1~FR-7 기능요구사항, 기술스택 결정사항, 범위, 범위 제외 항목 포함). 기능을 추가하기 전에 반드시 읽으세요 — 지금의 범위가 *왜* 이렇게 정해졌는지가 기록되어 있습니다(예: 엑셀 템플릿을 왜 고정 스키마로 했는지, 어떤 기능을 왜 의도적으로 제외했는지).
...
**아직 백엔드/데이터베이스가 없습니다.** 사용자가 등록하는 모든 "분석 요청"은 클라이언트 측 `localStorage`에만 `audit-ai-demo-requests` 키로 저장됩니다(`web/src/app/page.tsx` 참고). PRD에는 추후 Supabase 연동이 명시돼 있지만 아직 아무것도 연결되어 있지 않습니다.
...
세 경로 모두 동일한 구조로 정규화된 뒤 `requests` 목록에 들어가므로, 아직 구현되지 않은 후속 분석 로직(PRD FR-2 이후: 재무비율 분석, Benford/Beneish/Altman/RSF 이상탐지 모델, 감사 체크리스트 생성, MUS 샘플링, 대시보드/리포트 export)은 입력 출처와 무관하게 동작할 수 있습니다.

**필요한 환경변수** (`web/.env.local`, 커밋되지 않음): `DART_API_KEY`, `UPSTAGE_API_KEY`.
