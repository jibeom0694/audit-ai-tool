...
- `docs/개발노트.md` — **어떤 순서로 무엇을 겪으며 만들어졌는지**, 각 기능이 무엇을 보고 무엇을 못 보는지. 코드와 PRD만으로는 안 보이는 경위·설계 근거·알려진 함정이 정리돼 있습니다. 이 저장소를 처음 여는 사람(사람이든 AI든)은 `CLAUDE.md` 다음에 이 문서를 읽으면 가장 빠릅니다.
